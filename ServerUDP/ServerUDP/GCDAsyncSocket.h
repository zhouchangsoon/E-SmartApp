 ...
 * and the socket:didAcceptNewSocket: delegate method will be invoked. (delegate中激活的方法)
 * 
 ...
 * When a connection is accepted, a new instance of GCDAsyncSocket will be spawned to(产生) handle it,
 * and the socket:didAcceptNewSocket: delegate method will be invoked. (delegate 中的方法将被调用)
 *
 * The socket will listen on all available interfaces (e.g. wifi, ethernet, etc)
 **/
 ...
 * 
 * To not time out use a negative time interval.(没有延时则设置为负数，典型值-1)
 * 
 * This method will return NO if an error is detected, and set the error pointer (if one was given).(当有错误时返回值为NO)
 * Possible errors would be a nil host, invalid interface, or socket is already connected.
 * (错误有可能是空的host, 无效的接口， 或者socket已经连接)
 ...
 * 
 * If the socket is not already disconnected, an invocation to the socketDidDisconnect:withError: delegate method (如果socket还没有断开链接，则激发delegate中的方法)
 ...
 * 
 * Please note the recommended way of releasing a GCDAsyncSocket instance (e.g. in a dealloc method)    (将一个GCDAsyncSocket instance释放的示例)
 * [asyncSocket setDelegate:nil];
 * [asyncSocket disconnect];
 * [asyncSocket release];
 * 
 * If you plan on disconnecting the socket, and then immediately asking it to connect again,
 * you'll likely want to do so like this:  (将一个GCDAsyncSocket instance断开后重连)
 * [asyncSocket setDelegate:nil];
 * [asyncSocket disconnect];
...
// 
// When a read is complete the socket:didReadData:withTag: delegate method is dispatched on the delegateQueue. (当socket完成阅读时，会激发delegate中的方法)
// When a write is complete the socket:didWriteDataWithTag: delegate method is dispatched on the delegateQueue. (当socket完成写时，会激发delegate中的方法)
// 
// You may optionally set a timeout for any read/write operation. (To not timeout, use a negative time interval.)
// If a read/write opertion times out, the corresponding "socket:shouldTimeout..." delegate method
// is called to optionally allow you to extend the timeout.  (当读或写超过延时时，可以通过delegate中的方法来延长延时.
// Upon a timeout, the "socket:didDisconnectWithError:" method is called (超时后，delegate中的方法会调用)
// 
 ...
 * After completion, the data returned in socket:didReadData:withTag: will be a subset of the given buffer.  (当读完数据的时候，激发的方法)
 * That is, it will reference the bytes that were appended to the given buffer via
 * the method [NSData dataWithBytesNoCopy:length:freeWhenDone:NO].  (将数据加到buffer的方法)
**/
 ...
 * socket:didWriteDataWithTag: is invoked signifying that this particular write operation has completed. (当完成写时激发的方法)
